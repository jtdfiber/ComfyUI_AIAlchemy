...
class AlchemyWorkflow:
    ...
    def invalid_workflow(self):
        for node in self.workflow["nodes"]:
            ...
            if node_type == "AlchemyCryptoEncryptEnd":
                if self.crypto_bridge_node_id == 0:
                    self.crypto_bridge_node_id = int(node["id"])
                else:
                    raise ValueError(
                        "Error: Multiple 'AlchemyCryptoEncryptEnd' instances found."
                    )
                continue
            elif node_type == "AlchemyCryptoEncrypt":
                if self.save_crypto_node_id == 0:
                    self.save_crypto_node_id = int(node["id"])
                else:
                    raise ValueError(
                        "Error: Multiple 'AlchemyCryptoEncrypt' instances found."
                    )
                continue
        if self.save_crypto_node_id == 0:
            raise ValueError("Error: No 'AlchemyCryptoEncrypt' instances found.")
        if self.crypto_bridge_node_id == 0:
            raise ValueError("Error: No 'AlchemyCryptoEncryptEnd' instances found.")
    ...
    def output_workflow_simple_shell(self, output_workflow_name):
        ...
        if save_crypto_node is None:
            raise ValueError("AlchemyCryptoEncrypt not found in workflow")
        if crypto_bridge_node is None:
            raise ValueError("CryptoBridgeNode not found in workflow")
        save_crypto_node["type"] = "AlchemyCryptoDecrypt"
        ...
        save_crypto_node["properties"] = {"Node name for S&R": "AlchemyCryptoDecrypt"}
...
class AlchemyDecodeWorkflow:
    ...
    def calculate_input_anything_map(self):
        ...
        for node_id, node in self.prompt.items():
            if node.get("class_type") == "AlchemyCryptoDecrypt":
...
class WorkflowTrimHandler:
    ...
    def onprompt_handler(json_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for node in prompt.values():
            ...
            if class_type == "AlchemyCryptoEncrypt":
                has_new_component = True
                break
            if class_type == "AlchemyCryptoOutput":
        ...
        if has_new_component:
            ...
            if not user_token:
                if error_code == 401 or error_code == -3:
                    AuthUnit().login_dialog("Set your creator token in server.json to encrypt")
                    json_data["prompt"] = {}
                else:
                    PromptServer.instance.send_sync(
                        "alchemycrypto_toast",
                        {"content": f"Auth failed: {error_msg}", "type": "error"},
                    )
    ...
    def replace_prompt(prompt: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for node in prompt.values():
            if node.get("class_type") == "AlchemyCryptoImage":
        ...
        random_seed_node = next(
            (
                ...
                if node.get("class_type") == "AlchemyCryptoRandomSeed"
            ),
            None,
        )
        if random_seed_node:
