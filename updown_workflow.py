...
@routes.post("/alchemycrypto/message")
    ...
    def upload_workflow(self, template_id, temp_dir):
        ...
        if error_code == 1:
            ...
            if not auto_overwrite:
                json_content = {
                    "title": "A workflow with this id already exists. Overwrite?",
                    "icon": "info",
            ...
            PromptServer.instance.send_sync(
                "alchemycrypto_toast", {"content": f"Error: {error_msg}", "type": "error"}
            )
            return False
        combine_target_path = os.path.join(temp_dir, f"alchemycrypto_{template_id}.zip")
        combine_file_paths = []
        for file in [
        ...
        if success:
            PromptServer.instance.send_sync(
                "alchemycrypto_toast", {"content": "Upload succeeded", "type": "info", "duration": 5000}
            )
        else:
            PromptServer.instance.send_sync(
                "alchemycrypto_toast",
                {"content": f"Upload failed: {message}", "type": "error", "duration": 5000},
            )
        return success
...
class DownloadWorkflow:
    ...
    def download_workflow(self, template_id, serial_number_token, user_token):
        ...
        if success:
            ...
            if message == "need login" or message == "login required":
                AuthUnit().login_dialog("Set your creator token in server.json")
            else:
