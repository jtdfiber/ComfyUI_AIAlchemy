...
# AI Alchemy: point at the self-hosted licensing server (from server.json).
from .selfhost import server_url as _sh_url
DEFAULT_URL_PREFIX = _sh_url()
DEFAULT_WS_PREFIX = _sh_url().replace("https://", "wss://", 1).replace("http://", "ws://", 1)
...
class CatUrlConfig:
    ...
    def login_api_url(self):
        return self.get_server_url("/api/alchemycrypto/get_info")

    @property
    def workflow_url(self):
        return self.get_server_url(f"/api/alchemycrypto/workflow")

    @property
    def serial_number_url(self):
        return self.get_server_url(f"/api/alchemycrypto/serial_number")

    @property
    def user_client_workflow(self):
        return self.get_server_url(f"/api/alchemycrypto/client_workflow")
        ...
        if response.status_code == 500:
            try:
                ...
                error_msg = response_data.get(
                    "message", f"Unknown error (status_code={response.status_code})"
                )
            except Exception:
                error_msg = f"Server error (status_code={response.status_code})"
        else:
            error_msg = f"Download failed: status_code={response.status_code}"
        print(error_msg)
        return False, error_msg
    try:
        response_data = response.json()
    except ValueError:
        error_msg = "Could not parse server JSON"
        print(error_msg)
        return False, error_msg
    response_code = response_data.get("code", -1)
    if response_code != 0:
        error_msg = response_data.get("message", f"Unknown error code={response_code}")
        print(error_msg)
        return False, error_msg
    workflow_url = response_data.get("workflow_url", "")
    password = response_data.get("password", "")
    if not workflow_url or not password:
        error_msg = "Download failed: missing workflow_url or password"
        print(error_msg)
        return False, error_msg
    try:
        workflow_response = requests.get(workflow_url)
    except requests.RequestException as e:
        error_msg = f"Blob download error: {str(e)}"
        print(error_msg)
        return False, error_msg
    if workflow_response.status_code != 200:
        error_msg = f"Download failed: status_code={workflow_response.status_code}"
        print(error_msg)
        return False, error_msg
    try:
        ...
        error_msg = f"Decrypt failed: {str(e)}"
        print(error_msg)
        return False, error_msg
